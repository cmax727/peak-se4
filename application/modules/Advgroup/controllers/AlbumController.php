...
class Advgroup_AlbumController extends Core_Controller_Action_Standard {
  public function init()
  {
    if( !Engine_Api::_()->core()->hasSubject() )
    {
      if( 0 !== ($album_id = (int) $this->_getParam('album_id')) &&
          null !== ($album = Engine_Api::_()->getItem('advgroup_album', $album_id)) )
      {
  ...
  public function createAction(){
   ...
   if( !$form->isValid($this->getRequest()->getPost()) )
    ...
    try{
      ...
      $album->save();
      $db->commit();
      $this->_helper->redirector->gotoRoute(array('controller' => 'photo','action'=>'upload','subject'=>$group->getGuid(),'album_id' => $album->album_id), 'group_extended', true);
    }
    catch(Exception $e){
      $db->rollBack();
      throw($e);
  ...
  public function viewAction(){
    ...
    if($album->user_id != 0) $album_owner_id = $album->user_id;
      else $album_owner_id = $group->user_id;
    $group_owner_id = $group->user_id;
    if($viewer->getIdentity()==0){
      $this->view->canEdit = false;
    }
    else{
      if($viewer->level_id == 1 || $viewer->getIdentity()==$album_owner_id || $viewer->getIdentity() == $group_owner_id){
  ...
  public function listAction(){
    ...
    if($viewer->getIdentity()==0) $form->removeElement ('view');
  ...
  public function deleteAction(){
     ...
     catch( Exception $e )
    {
     ...
     return $this->_forward('success', 'utility', 'core', array(
          ...
          'layout' => 'default-simple',
          'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('controller' => 'album', 'action' => 'list','subject' => $group->getGuid(),'album_id'=>$album->getIdentity()),'group_extended',true),
          'closeSmoothbox' => true,
        ));
  }
}
