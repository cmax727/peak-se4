...
class Advgroup_PollController extends Core_Controller_Action_Standard{
  public function init()
  {
    ...
    {
      if( 0 !== ($poll_id = (int) $this->_getParam('poll_id')) &&
          null !== ($poll = Engine_Api::_()->getItem('advgroup_poll', $poll_id)) )
      {
  ...
  public function listAction(){
    ...
    $form->populate(array('search' => $params['search'],
   ...
   //Create Checking
    ...
    if($canCreate && $levelCreate) $this->view->canCreate = true;
      else $this->view->canCreate = false;
  ...
  public function editAction(){
    ...
    if(!$poll) return $this->renderScript ('_error.tpl');

    //Edit condition
    $group = $poll->getParent();
    $canEdit = $group->authorization()->isAllowed(null,  'poll.edit');
    if(!$canEdit && !$poll->isOwner($viewer)){
    ...
    $form->populate(array('title'=>$poll->title,
  ...
  public function deleteAction(){
    ...
    $this->view->message = Zend_Registry::get('Zend_Translate')->_('The poll has been successfully deleted.');
     return $this->_forward('success' ,'utility', 'core', array(
      'parentRedirect'=> Zend_Controller_Front::getInstance()->getRouter()->assemble(array('controller'=>'poll','action'=>'manage','subject'=>$group->getGuid()),'group_extended',true),
      'messages' => Array($this->view->message)
    ));
  }
  ...
  public function closeAction(){
    ...
    if($this->_getParam('closed')) $this->view->form = $form = new Advgroup_Form_Poll_Close;
    else $this->view->form = $form = new Advgroup_Form_Poll_Unclose;
    ...
    $this->view->status = true;
    if($poll->closed) $this->view->message = Zend_Registry::get('Zend_Translate')->_('The poll has been closed.');
    else $this->view->message = Zend_Registry::get('Zend_Translate')->_('The poll has been reopened.');
    return $this->_forward('success' ,'utility', 'core', array(
      'parentRefresh'=>true,
      'messages' => Array($this->view->message)
    ));
  ...
  if( !$this->_helper->requireAuth()->setAuthParams($group, null, 'poll')->isValid() ) return;
