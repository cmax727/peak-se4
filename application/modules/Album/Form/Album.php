 ...
 * @version    $Id: Album.php 9238 2011-09-05 23:33:47Z shaun $
 * @author     Sami
 */
...
class Album_Form_Album extends Engine_Form
{
  public function init()
  {
	...
	//?? -- available tied network
	if ( $user->isAdmin()){
		$ownerSelect = Engine_Api::_()->getDbtable('networks', 'network')->select();
	}else{
		$ownerSelect = Engine_Api::_()->getDbtable('membership', 'network')->getMembershipsOfSelect($user);
	}
	$ownerSelect->order('title');
	
	$ownerNetworks = Engine_Api::_()->getDbtable('networks', 'network')->fetchAll($ownerSelect);
	$networkMultiOptions = array();
	foreach( $ownerNetworks as $row ) {
	  $networkMultiOptions[$row->network_id] = $row->getTitle();
	}
	$this->addElement('Multiselect', 'tied_network', array(
			'label' =>  'Tied Network(s)',
			'required' => true,
			'validators' => array(
				array('NotEmpty', true),
				),
			'description' => 'Which network(s) should this album be tied to? Select multiple by holding down the Ctrl or Command key.',
			'multiOptions' => $networkMultiOptions,
			'value' => key($networkMultiOptions),
		));
	$this->tied_network->getDecorator('Description')->setOption('placement', 'append');
  ...
  public function saveValues()
  {
	...
	if( ($values['album'] == 0) )
	{
	  ...
	  //??
	  $album->setTiedNetworks($values['tied_network']);
