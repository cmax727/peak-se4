 ...
 * @version    $Id: AlbumController.php 9362 2011-10-10 22:25:15Z john $
...
class Album_AlbumController extends Core_Controller_Action_Standard
{
  ...
  public function editAction()
  {
	...
	if (! $album->isAllowed(Engine_Api::_()->user()->getViewer(), 'edit')) {
	...
	if( !$this->getRequest()->isPost() )
	{
	  ...
	  $nets = $album->getTiedNetworks();
	  $netIds = array();
	  foreach ($nets as $net){
		$netIds[] = $net->network_id;  
	  }
	  $form->tied_network->setValue($netIds);
  ...
  public function viewAction()
  {
	...
	if ( false ==$album->isAllowed($viewer, 'view')) {
		$this->_forward("requireauth", "error", "core");
		return;
	}
	//??if( !$this->_helper->requireAuth()->setAuthParams($album, null, 'view')->isValid() ) return;
	...
	$this->view->canEdit = $album->isAllowed($viewer, 'edit');
  ...
  public function deleteAction()
  {
	...
	if( false == $album->isAllowed($viewer, 'delete')){
		$this->_forward("requireauth", "error", "core");
		return;
	}
	// In smoothbox
  ...
  public function editphotosAction()
  {
	...
	if ( false == $album->isAllowed($viewer, 'edit')) {
		$this->_forward("requireauth", "error", "core");
		return;
	}
	//??if( !$this->_helper->requireAuth()->setAuthParams(null, null, 'edit')->isValid() ) return;
  ...
  public function orderAction()
  {
	...
	$album = Engine_Api::_()->core()->getSubject();
	if(true != $album->isAllowed(Engine_Api::_()->user()->getViewer(), "edit") ) {
		$this->_forward("requireauth", "error", "core");
		return;
	}
