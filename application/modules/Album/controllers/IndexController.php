...
//	$user_id = $this->_getParam('user');
	...
	$albumPermi = Engine_Api::_()->authorization()->getPermission($viewer, 'album', 'view') ;
	
	if ( $viewer->isAdmin() ||
		$albumPermi == 2
		){
		//$select->where("owner_id = '%%'");
	}else if ($albumPermi == 1){
		$mTable_ = Engine_Api::_()->getDbtable('membership', 'network');
		$mTable = $mTable_->info('name');
		
		//$user_nIds = $mTable->getMembershipsOfIds($viewer);
		
		$id_select = $mTable_->select()->from(array('m'=>$mTable), array('user_id',) );
		$id_select->where("m.resource_id in (select distinct n.resource_id from $mTable n
			where  n.user_id = '{$viewer->getIdentity()}')
			"
		);
		$owner_ids = $id_select->query()->fetchAll();
		
		$select->where("owner_id in (?)", $owner_ids);
	}else{ // not allowed
		$select->where("owner_id = null");
	}
