  ...
  public function init()
  {
	...
	if( !$this->isPublic() ) {
	  ...
	  */
	  ...
	  //??2-7 Element: messages_auth_NEW
	  $this->addElement('Radio', 'messages_auth2', array(
		'label' => 'Who can member send messages to?',
		'description' => 'USER_FORM_ADMIN_SETTINGS_LEVEL_MESSAGESAUTH_DESCRIPTION',
		'multiOptions' => array(
		  'everyone' => 'message to all-all',
		  'users_net' => 'message to all-usernet',
		  'users' => 'message to all-none',
		  'none' => 'Disable messaging',
		),
		'value' => 'users', // users in same networks
	  ));
	  $this->messages_auth2->getDecorator('Description')->setOption('placement', 'PREPEND');
	}
  }
}