...
class Authorization_AdminLevelController extends Core_Controller_Action_Admin
{
  ...
  public function editAction()
  {
	...
	$messagesAuth2 = $permissionsTable->getAllowed('messages', $id, 'auth2');
	$form->populate(array(
	  'messages_auth' => $messagesAuth,
	  'messages_auth2' => $messagesAuth2,
	));
	...
	$messageAuth2 = $values['messages_auth2'];
	unset($values['messages_auth']);
	unset($values['messages_auth2']);
	...
	//  'create' =>  in_array($messageAuth, array('everyone' ,'friends' )),
	  'create' =>  in_array($messageAuth2, array('everyone' ,'users_net', 'users' )),
	  'auth' => $messageAuth,
	  'auth2' => $messageAuth2,
...
public function editGalleryAction()
  {
	...
	$this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
	  ->getNavigation('authorization_admin_main', array(), 'authorization_admin_main_album_level');
	//check if Album module exist
	$moduleTable = Engine_Api::_()->getDbTable("modules", "core");
	if (! $moduleTable->isModuleEnabled("album")){ // ** see $moduleTable->hasModule("album") && 
		//$this->_helper->viewRenderer->setNoRender();
		$this->view->noPlugin = true;
		return;
	}
	...
	$this->view->form = $form = new Authorization_Form_Admin_Level_Editgallery(array(
	  'public' => ( in_array($level->type, array('public',)) ),
	  'moderator' => ( in_array($level->type, array('admin', 'moderator')) ),
	  'usertype' => $level->type,
	));
