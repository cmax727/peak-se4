...
class Fields_Form_Standard extends Engine_Form
{
  ...
  public function saveValues(){
	  $this->saveFormValues($this->getItem(), $this);
  }
  
  //??
  public function saveFormValues($item, $form)
  {
	...
	if( $form->_elementsBelongTo ) {
	  if( isset($fVals[$form->_elementsBelongTo]) ) {
		$fVals = $fVals[$form->_elementsBelongTo];
	  }
	}
	...
	foreach( $fVals as $key => $value )
	{
	  ...
	  // Whoops no headings
	  if( $form->getElement($key) instanceof Engine_Form_Element_Heading )
	  {
		continue;
	  }

	  // Array mode
	  if( is_array($value) || $form->getElement($key)->isArray() ) {
		...
		if( is_array($value) || !empty($value) ) {
		  foreach( (array) $value as $singleValue ) {
			...
			$valueRow->item_id = $item->getIdentity();
	  ...
	  else
	  {
		...
		if( !$isNew ) {
		  $field = Engine_Api::_()->fields()->getFieldsMeta($item)->getRowMatching('field_id', $field_id);
		  if( is_object($field) && !empty($field->publish) ) {
			...
			if( $field->type )
			$actionTable->addActivity($item, $item, 'fields_change_generic', null, array(
			  'label' => $field->label,
			  'value' => $helper->getFieldValueString($field, $valueRow, $item), //$value,
			));
		  }
		}
		 * 
	...
	// Update search table
	Engine_Api::_()->getApi('core', 'fields')->updateSearch($item, $values);
