 ...
 * @version    $Id: Search.php 8790 2011-04-05 22:08:54Z john $
 * @author     John
 */
...
class Fields_Model_DbTable_Search extends Fields_Model_DbTable_Abstract
{
  ...
  public function getSearch($item)
  {
	...
	return $this->getSearchByIdentity($id);
  }
  //??
  public function getSearchByIdentity($identity)
  {
	$id = $identity;
  ...
  public function checkSearchIndex($field)
  {
	...
	$fieldValues  = $table->fetchAll($table->select()->where('field_id = ?', $field->field_id));
	foreach ( $fieldValues as $fv){
		$search = $this->getSearchByIdentity($fv->item_id);
		$search->$searchCol = $fv->value;
		$search->save();
	}
