...
//        ;
	...
	// Make form
	$this->view->form = $form = new Messages_Form_Compose($viewer);
	// Build
	$isPopulated = false;
...
//        }
	  ...
	  // Normal
		...
		if ( $values['toType'] == "0"){ // normal user
			...
			foreach ( $recipients as $k=>$targetId){
				if ( !Messages_Model_Message::isMsgAllowed($viewer, $targetId, "user"))	
					unset($recipients[$k]);
			}
			
		}else{ // to network
			$recipientNets = $values['toNetworks'];
			foreach ( $recipientNets as $k=>$net){
				if ( !Messages_Model_Message::isMsgAllowed($viewer, $net, "network"))
					unset($recipientNets[$k]);
			}
			
			foreach ( $recipientNets as $k=>$targetId){
				if ( !Messages_Model_Message::isMsgAllowed($viewer, $targetId, "network"))	
					unset($recipientNets[$k]);
			}
			$recipientNets = implode(", ", $recipientNets);
			
			$netTable = Engine_Api::_()->getDbtable('membership', 'network');
			$sendTos = $netTable->select()->where("
					active = '1' and 
					user_id <> {$viewer->user_id} and 
					resource_id in ($recipientNets)" )->query()->fetchAll();
			foreach($sendTos as $row){
		...
		// Validate friends
		// @todo here I have to validate ?? saf 
		/*if( 'friends' == Engine_Api::_()->authorization()->getPermission($viewer, 'messages', 'auth') ) {
