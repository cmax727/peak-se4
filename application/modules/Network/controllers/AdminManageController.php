 ...
 * @version    $Id: AdminManageController.php 8558 2011-03-04 01:12:51Z jung $
 * @author     Sami
 * @author     John
 */
...
class Network_AdminManageController extends Core_Controller_Action_Admin
{
  ...
  public function indexAction()
  {
	...
	$select2->order('order');
	$networkList = $networkTable->fetchAll($select2); //$select->query()->fetchAll();
	
	$networkData = array();
	
	foreach( $networkList as $net){
		$networkData[] = array('id'=> $net->network_id,
				'title' => $net->title);
	}
	$this->view->networks = $networkData;
	// Apply params
...
/*
select u.* from engine4_users u
where u.user_id in (
  select distinct m.user_id from engine4_network_membership m
  where ( m.resource_id in
  (
   select m2.resource_id from engine4_network_membership m2 where m2.user_id = 36
  )
 )
 )
;
