...
<script type="text/javascript">
  ...
  var uiToggleOptions = function(spec, forceClose) {
   ...
   window.addEvent('domready', function() {
	$('network-order').removeEvents().addEvent('click', uiToggleOptions);
...
<div>
  ...
  //??
  //add network link
   echo $this->htmlLink(array('action' => 'create', 'reset' => false), $this->translate('Add Network'), array(
	'class' => 'buttonlink',
  ...
  /* 
  //?? network order link
  echo "<a id='network-order' href='javascript:void(0);' class='buttonlink' style = 'background-image: url('" . $this->layout()->staticBaseUrl . 'application/modules/Network/externals/images/admin/add.png);'
  . "> {$this->translate('Network Order')}</a>";
  
  echo "<div class='field_extraoptions'>";
  echo "<div class='field_extraoptions_contents_wrapper'>";
  echo "<div class='field_extraoptions_contents'>";
  echo "<ul class='field_extraoptions_choices' style>";
  
   foreach ( $this->networks as $net){
	   echo "<li class='field_option_select'> 
		<span class='field_extraoptions_choices_label' network_id='{$net['id']}'> {$net['title']} </span></li>";
   }
  echo "</ul></div></div></div>";
  */
   ?>
