...
class User_Form_Signup_Account extends Engine_Form
{
  public function init()
  {
    ...
    $loginID = Zend_Registry::get('Zend_Translate')->_('Login ID'); // ??
    // Element: Login ID
    $this->addElement('Text', 'ulogin_id', array(
      'label' => $loginID,
      'description' => 'You will use this ID to login.',
      'required' => true,
      'allowEmpty' => false,
      'validators' => array(
        'alnum',
        array('NotEmpty', true),
        array('Db_NoRecordExists', true, array(Engine_Db_Table::getTablePrefix() . 'users', 'ulogin_id')) //check existance
      ),
      'filters' => array(
        'StringTrim'
      ),
	...
	//login ID decorate /validate ??
	$this->ulogin_id->getDecorator('Description')->setOptions(array('placement' => 'APPEND'));
    $this->ulogin_id->getValidator('NotEmpty')->setMessage('Please enter a valid Login ID with alpha numerical expression.', 'isEmpty');
    $this->ulogin_id->getValidator('Db_NoRecordExists')->setMessage('Someone has already registered this Login ID, please use another one.', 'recordFound');
    
    //email address decorate /validate ??    
