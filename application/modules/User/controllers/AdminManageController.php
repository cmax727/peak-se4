...
class User_AdminManageController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
	...
	$select = $table->select()->from(array('u'=>$table->info('name')));

	//?? profile type search
	$profile_id = 1;
	if( !empty($values['profile_type']) ) {
	
	  $fieldTbName= Engine_Db_Table::getTablePrefix()."user_fields_values";
	  
	  $select->join(array('p'=>$fieldTbName),
			  "p.item_id = u.user_id and p.field_id = $profile_id "
			 ,array()//,  array("profile_type" => "p.value")
			  )
	  ->joinInner(array('o'=>Engine_Db_Table::getTablePrefix()."user_fields_options"),
			"o.option_id = p.field_id"
			,array()//, array("profile_name" => "o.label")
			)
	  ->where ("p.value = ?",$values['profile_type']);
	...
	// Set up select info ( default = ulogin_id)
	$select->order(( !empty($values['order']) ? $values['order'] : 'ulogin_id' ) . ' ' . ( !empty($values['order_direction']) ? $values['order_direction'] : 'DESC' ));
  ...
  public function editAction()
  {
	...
	//surbForm ( 'profileForm') is automatically populated by saf
...
//	unset($values['network_id']);
	...
	// ??
	// set Profile values {
	$form->saveFormValues($user, $form->getSubForm("profileForm"));  
