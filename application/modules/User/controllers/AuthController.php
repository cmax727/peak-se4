...
//    }
	...
	// Check login creds
	extract($form->getValues()); // $email, $password, $remember, $ulogin_id
	$user_table = Engine_Api::_()->getDbtable('users', 'user');
	$user_select = $user_table->select()
	  ->where('ulogin_id = ?', $ulogin_id);          // If post exists  0= $email
	...
	if( !$user->enabled ) {
	  if( !$user->verified ) {
		...
		$resend_url = $this->_helper->url->url(array('action' => 'resend', 'ulogin_id'=>$ulogin_id), 'user_signup', true);
	...
	// Normal authentication
	  ...
	  $authResult = Engine_Api::_()->user()->authenticate($ulogin_id, $password);
  ...
  public function logoutAction()
  {
	...
	if( !empty($_SESSION['login_id']) ) {
	  ...
	  unset($_SESSION['login_id']); //user_logins : login_id 
	  unset($_SESSION['ulogin_id']); //user_logins : ulogin_id
  ...
  public function forgotAction()
  {
	...
	$userList = $userTable->fetchAll(array('email = ?' => $form->getValue('email')));
	//??
	
	if( !$userList ) { // || !$user->getIdentity() 
	  $form->addError('Any user account with that Login ID was not found.');
	  return;
	}
	...
	if( !$isValidExist) {
	  $form->addError("User account(s) under email address {$form->getValue('email')} has not yet been verified or disabled by an admin.");
	  return;
	}
	...
	try
	{
	 ...
	 $forget_info = $translate->_("_EMAIL_USER_FORGET_INFO");
	 $forget_list = array();
	 $host = $_SERVER['HTTP_HOST'];			  
	 
	 foreach ( $userList as $user){
		 if ( !$user->enabled){
			 $resetlink = "That user account has not yet been verified or disabled by an admin.";
		 }else{
			 ...
			 $forgotTable->insert(array(
			  ...
			  $resetlink = "http://[host][object_link]";
			  
			  $resetlink = str_replace("[host]", $host, $resetlink);
			  $resetlink = str_replace("[object_link]", 		
						$this->_helper->url->url(array('action' => 'reset', 'code' => $code, 'uid' => $user->getIdentity())),
						 $resetlink); 	 
		 }
		 $reset_info = str_replace("[ulogin_id]", $user->ulogin_id, $forget_info);
		 $reset_info = str_replace("[reset_link]", $resetlink, $reset_info);
		 
		 $forget_list[] = $reset_info;
		  
	  }
	  // Send user an email

		  //?? send the last founded user name
