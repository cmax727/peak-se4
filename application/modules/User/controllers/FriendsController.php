...
*/
	  ...
	  $ids = array();
	  //?? new select here  :saf
	  $msgPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'messages', 'auth2') ;
	  ...
	  if ( $msgPermission == 'everyone' || $user->isAdmin()){
	  ...
	  }else if ( $msgPermission =='users_net' || $msgPermission =='users'){
		$select = $userTable->select()->from(array('u'=>$userTable->info('name')));
		...
		$select =  Engine_Api::_()->getItemTable('user')->select()->where("1=2");    
	  }
	  
	  
	  if( null !== ($text = $this->_getParam('search', $this->_getParam('value'))) ) {
		$select->where('u.displayname LIKE ?', '%'. $text .'%');
	  }
	  
	  $select->order('displayname');
	  
	  //$select->getTable()
	  foreach( $userTable->fetchAll($select) as $friend ) {
		  // get user's  network list
		$Nselect = Engine_Api::_()->getDbtable('membership', 'network')->getMembershipsOfSelect($friend);
		$networks = Engine_Api::_()->getDbtable('networks', 'network')->fetchAll($Nselect);
		$netIds = array();
		$netNames = array();
		foreach( $networks as $network ) {
		  $netIds []= $network->getIdentity();
		  $netNames [] = $network->title;
		}
		$strNetNames = implode(", ", $netNames);
		
		$data[] = array(
		  'type'  => 'user',
		  'id'    => $friend->getIdentity(),
		  'guid'  => $friend->getGuid(),
		  'label' => $friend->getTitle()." => {$strNetNames}",
